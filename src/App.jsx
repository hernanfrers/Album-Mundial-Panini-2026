...
export default function PaniniTracker() {
  ...
  const toggleAllTeam = (team) => {
    setCollected(prev => {
      ...
      return { ...prev, [team]: new Array(STICKERS_PER_TEAM).fill(!all) };
    });
  };
  ...
  const exportAndPrint = (collectedState) => {
    const rows = Object.entries(GROUPS).map(([groupKey, teams]) => {
      const teamRows = teams.map(team => {
        ...
        return `<tr>
          <td class="team-cell">${FLAGS[team] || ""} ${team}</td>
          <td class="stickers-cell">${stickers}</td>
          <td class="count-cell">${cnt}/20</td>
        </tr>`;
      }).join("");
    ...
    <title>Panini Mundial 2026</title>
    <style>
      ...
      .group-row td { background: #1a1a3e; color: white; font-weight: bold; font-size: 12px; padding: 6px 8px; letter-spacing: 2px; }
      .team-cell { width: 160px; font-weight: bold; white-space: nowrap; }
      .count-cell { width: 40px; text-align: right; color: #888; }
      .stk { display: inline-block; width: 22px; height: 22px; border: 1px solid #ccc; border-radius: 4px; text-align: center; line-height: 22px; margin: 1px; font-size: 10px; color: #bbb; }
      .stk.got { background: #ffd700; border-color: #ffd700; color: #333; font-weight: bold; }
      @media print { body { margin: 10px; } }
    </style></head><body>
    <h1>🏆 PANINI · COPA MUNDIAL FIFA 2026</h1>
    <div class="sub">${total} / 960 figuritas recolectadas (${Math.round(total/960*100)}%) · USA · MÉXICO · CANADÁ</div>
    <table>${rows}</table>
    </body></html>`;
  ...
  const resetAll = () => {
    if (confirm("¿Resetear todo el progreso?")) setCollected(initialState());
  };
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .filter-btn.active {
          background: #ffd700; color: #1a1a00;
          border-color: #ffd700;
        }
        ...
        @media print {
          ...
          .sticker-btn { border-color: #ccc !important; color: #ccc !important; background: white !important; box-shadow: none !important; }
          .sticker-btn.collected { background: #ffd700 !important; color: black !important; border-color: #ffd700 !important; }
          .team-card { background: #f9f9f9 !important; border-color: #ddd !important; page-break-inside: avoid; }
          div[style*="linear-gradient"] { background: white !important; }
        }
      `}</style>

      {/* HEADER */}
      <div style={{
        background: "linear-gradient(180deg, #0a0a2e 0%, #1a1050 100%)",
        borderBottom: "3px solid #ffd700",
        padding: "28px 20px 24px",
        textAlign: "center",
        position: "relative",
      }}>
        <div style={{ fontSize: 13, color: "#ffd700", letterSpacing: 4, fontFamily: "'Lato', sans-serif", fontWeight: 700, marginBottom: 6, textTransform: "uppercase" }}>
          🏆 Copa Mundial de la FIFA™
        </div>
        <div style={{ fontFamily: "'Oswald', serif", fontSize: "clamp(28px, 7vw, 52px)", fontWeight: 700, letterSpacing: 2, lineHeight: 1 }}>
          PANINI 2026
        </div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", fontFamily: "'Lato', sans-serif", marginTop: 6, letterSpacing: 1 }}>
          RASTREADOR DE FIGURITAS · USA · MÉXICO · CANADÁ
        </div>

        {/* Global progress */}
        <div style={{ maxWidth: 500, margin: "20px auto 0", padding: "0 10px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ fontFamily: "'Lato', sans-serif", fontSize: 13, color: "rgba(255,255,255,0.6)" }}>
              {totalCollected} / {totalStickers} figuritas
            </span>
            <span style={{ fontFamily: "'Oswald', sans-serif", fontSize: 18, color: "#ffd700", fontWeight: 700 }}>
              {pct}%
            </span>
          </div>
      ...
      {/* CONTROLS */}
      <div className="no-print" style={{ padding: "16px 16px 0", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button
            onClick={() => exportAndPrint(collected)}
            style={{ marginLeft: "auto", padding: "7px 18px", borderRadius: 20, border: "1px solid rgba(255,255,255,0.3)", background: "transparent", color: "rgba(255,255,255,0.7)", cursor: "pointer", fontFamily: "'Lato', sans-serif", fontSize: 13 }}
          >
            🖨️ Imprimir
          </button>
          <button
            onClick={resetAll}
            style={{ padding: "7px 18px", borderRadius: 20, border: "1px solid rgba(255,80,80,0.4)", background: "transparent", color: "rgba(255,120,120,0.7)", cursor: "pointer", fontFamily: "'Lato', sans-serif", fontSize: 13 }}
          >
            🗑️ Reset
          </button>
        </div>
      ...
      {/* LEGEND */}
      <div className="no-print" style={{ padding: "10px 16px 0", maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "flex", gap: 16, fontSize: 12, color: "rgba(255,255,255,0.4)", fontFamily: "'Lato', sans-serif" }}>
          ...
          <span>Hacé click en cada número para marcar</span>
        </div>
      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "16px 16px 0" }}>
        {groups.map(([groupKey, teams]) => {
          ...
          const filteredTeams = showOnlyMissing
            ? teams.filter(t => !collected[t].every(Boolean))
            : teams;
            ...
            <div key={groupKey} style={{ marginBottom: 28 }}>
              <div className="group-header">
                <div style={{
                  ...
                  fontFamily: "'Oswald', serif", fontSize: 18, fontWeight: 700, color: "white",
                  flexShrink: 0, boxShadow: `0 0 16px ${color}88`,
                }}>
                  {groupKey}
                </div>
                <div style={{ fontFamily: "'Oswald', serif", fontSize: 20, letterSpacing: 2, color }}>
                  GRUPO {groupKey}
                </div>
                <div style={{ flex: 1, height: 1, background: `linear-gradient(90deg, ${color}44, transparent)` }} />
                <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 12, color: "rgba(255,255,255,0.4)" }}>
                  {teams.reduce((s, t) => s + collected[t].filter(Boolean).length, 0)} / {teams.length * STICKERS_PER_TEAM}
                </div>
              </div>
              ...
              {filteredTeams.map(team => {
                ...
                const isExpanded = expandedTeams[team] !== false; // default expanded
                  ...
                  <div key={team} className="team-card">
                    <div className="team-header" onClick={() => toggleTeam(team)}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "'Oswald', serif", fontSize: 16, letterSpacing: 1, marginBottom: 5 }}>
                          {team}
                        </div>
                        <div className="progress-mini">
                          <div className="progress-fill" style={{
                            width: `${teamPct}%`,
                            background: cnt === STICKERS_PER_TEAM
                              ? "linear-gradient(90deg, #00c851, #00e676)"
                              : `linear-gradient(90deg, ${color}, ${color}99)`,
                          }} />
                        </div>
                      </div>
                      <div style={{ fontFamily: "'Lato', sans-serif", fontSize: 12, color: cnt === STICKERS_PER_TEAM ? "#00e676" : "rgba(255,255,255,0.5)", minWidth: 50, textAlign: "right" }}>
                        {cnt === STICKERS_PER_TEAM ? "✅ COMPLETO" : `${cnt}/${STICKERS_PER_TEAM}`}
                      </div>
                      ...
                      <div style={{ padding: "0 16px 14px" }}>
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 10 }}>
                          {collected[team].map((has, i) => (
                            <button
                              key={i}
                              className={`sticker-btn ${has ? "collected" : ""}`}
                              onClick={() => toggle(team, i)}
                              title={has ? `Figurita ${i + 1} — Tenés` : `Figurita ${i + 1} — Falta`}
                            >
                              {i + 1}
                            </button>
                          ))}
